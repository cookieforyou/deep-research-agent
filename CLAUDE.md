...
├── api/                                        # 接口层 (WebFlux + SSE)
│   ├── controller/
│   │   ├── ResearchController.java             # REST 研究接口
│   │   ├── ResearchHistoryController.java      # 研究历史 CRUD
│   │   ├── UserProfileController.java          # 当前用户画像
...
│   │   └── research/[sessionId]/page.tsx      # 研究结果详情
│   ├── login/page.tsx                         # Casdoor OAuth2 登录
│   └── page.tsx                               # 首页（研究输入）
├── src/components/
│   ├── admin/                                 # 管理组件（6 个）
│   │   ├── PromptTable.tsx                    # 模板列表表格
│   │   ├── PromptEditor.tsx                   # 模板编辑弹窗（含对比）
│   │   ├── PromptDiffModal.tsx                # LCS 算法逐行对比
│   │   ├── PromptStatusBadge.tsx              # 状态徽章
│   │   ├── AbTestConfig.tsx                   # A/B 分组批量配置
│   │   └── UserTable.tsx                      # 用户管理表格（可展开兴趣标签）
│   ├── history/                               # 历史组件（5 个）
│   ├── layout/                                # 布局组件（5 个：Navbar/Sidebar/AuthGuard/Breadcrumb/Footer）
│   ├── research/                              # 研究组件（15 个）
│   └── ui/                                    # shadcn/ui 基础组件（18 个）
├── src/hooks/                                 # 自定义 Hooks（9 个）
│   ├── usePromptManagement.ts                 # 管理员 Prompt CRUD + 批量 AB 分组
│   ├── useUserManagement.ts                   # 管理员用户列表
│   ├── useResearchSse.ts                      # SSE 连接管理（指数退避重连）
│   ├── useHistoryList.ts                      # 研究历史列表
│   └── ...
├── src/stores/                                # Zustand 状态（3 个）
│   ├── auth-store.ts                          # JWT 认证（Casdoor 集成）
│   ├── sse-store.ts                           # SSE 事件缓冲（max 200/session）
│   └── ui-store.ts                            # UI 状态（侧边栏折叠等）
└── src/lib/
    ├── api.ts                                 # API 客户端（全部后端端点）
    ├── types.ts                               # TypeScript 类型（与后端 DTO 对齐）
    ├── constants.ts                           # 阶段标签、示例查询、评估维度
    ├── jwt.ts                                 # JWT Token 解析/刷新
    ├── sse.ts                                 # SSE 客户端（fetch-event-source）
...
以下端点需要 `ROLE_ADMIN` 权限，由 `PromptAdminController` 和 `UserAdminController` 提供。

| 方法 | 路径 | 说明 |
|:---|:---|:---|
| GET | `/api/admin/prompts` | 全部 Prompt 模板列表 |
| GET | `/api/admin/prompts/{id}` | 单个模板详情 |
| PUT | `/api/admin/prompts/{id}` | 更新模板（内容/状态/AB分组，字段可选） |
| POST | `/api/admin/prompts/{id}/reset` | 重置为 classpath 默认值 |
| POST | `/api/admin/prompts/{id}/cache/invalidate` | 强制刷新本地缓存 |
| POST | `/api/admin/prompts/batch-ab-group` | 批量更新 A/B 分组 |
| GET | `/api/admin/users?page=&size=&search=` | 分页查询用户画像列表 |

Prompt 模板通过 `DynamicPromptService` 三阶加载（内存缓存 1min TTL → PostgreSQL `prompt_templates` 表 → classpath `.st` 兜底）。管理员修改后自动生效。

用户画像数据在研究过程中自动收集（`LongTermMemoryService`），包括 `interests`（兴趣标签）、`preferences`（偏好）、`recentTopics`（最近主题）、`researchCount`（研究次数）。`PreferenceExtractorAgent` 异步提取稳定偏好并合并入 `user_profile.preferences`。
...
### 添加管理后台 API
1. 在 `api/controller/` 下创建 Controller 类，加 `@PreAuthorize("hasRole('ADMIN')")`
2. 所有方法返回 `Mono<ResponseEntity<T>>`（WebFlux 响应式 + 方法级权限拦截器兼容）
3. 如需新 DTO，在 `api/dto/` 下创建 Java Record
4. 需要在 `UserProfileRepository` 等 Repository 添加对应查询方法

### 添加前端管理页面
1. 在 `ui/src/app/(admin)/admin/` 下创建 Next.js 页面
2. 在 `ui/src/components/admin/` 下创建对应组件
3. 在 `ui/src/hooks/` 下创建 TanStack Query hook（参考 `usePromptManagement.ts`）
4. 在 `ui/src/lib/api.ts` 的 `adminApi` 中添加 API 请求函数
5. 在 `ui/src/lib/types.ts` 中添加 TypeScript 接口（与后端 DTO 对齐）
6. 在 `ui/src/app/(admin)/layout.tsx` 的 `ADMIN_LINKS` 中注册侧边栏入口
