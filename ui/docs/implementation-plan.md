...
| 3 | 实时进度与工作流可视化 | 2.5 天 | 无（复用已有 SSE） | Phase 2 | ✅ 已完成 |
| 4 | 报告渲染与引用展示 | 2.5 天 | **需新增 1 个 API** | Phase 3 | ✅ 已完成 |
| 5 | 评估展示与研究历史 | 3 天 | **需新增 3 个 API** | Phase 4 | ✅ 已完成 |
| 6 | 管理后台 | 2 天 | **需新增 5 个 API** | Phase 1 (独立) | ✅ 已完成 |
| 7 | 打磨、测试与部署 | 3 天 | 无（联调修复） | Phase 5+6 | ✅ 已完成 |
| **合计** | | **17 天** | **10 个新 API** | | |
...
| 项目 | 计划 | 实际 | 原因 |
|:---|:---|:---|:---|
| SearchProgress 组件 | 独立 `<SearchProgress>` 组件 | 内置为 `WorkflowNode.SearchChildNode` | 搜索进度与 dual_search 节点强耦合，作为子组件更合理 |
| 节点状态计算 | 简单 `pending/active/done` | 增加 error 状态 + 实时计时器 | 节点可能因下游错误标记为 error；活跃节点每秒刷新耗时 |
| 降级通知 | Toast | useMemo 副作用触发 toast + toast id 去重 | MODEL_FALLBACK/SEARCH_FALLBACK 事件触发 sonner toast，相同 id 只显示一次 |
| `isCacheHit` prop | 传入 WorkflowTimeline | 父组件条件渲染 | `isCacheHit` 时完全不渲染 Timeline（显示 CacheHitBanner），无需传入 |
| SSE 事件调试面板 | 保留 | 已移除（替换为 Timeline） | Phase 3 正式替代 Phase 2 调试占位 |
| dual_search 子节点进度 | 解析 `percent` 字段 | 正则解析 `message` 中的 `x/N` 格式 | 后端 `ProgressEvent.searching()` 的 message 格式为 "Web搜索: 4/6" |
...
src/
└── components/research/
    ├── WorkflowTimeline.tsx        # ★ 核心：状态计算引擎 + 7 节点渲染 + 实时计时器
    ├── WorkflowNode.tsx            # ★ 单节点组件（4 状态 + 圆圈连线 + SearchChildNode）
    ├── CacheHitBanner.tsx          # ★ 语义缓存命中横幅
    ├── ResearchErrorView.tsx       # ★ 研究失败展示（错误信息 + 重试 + 返回首页）
    └── app/(research)/research/[sessionId]/
        └── page.tsx                # 更新：Timeline + CacheHitBanner + ErrorView 集成
```
...
| # | 验收标准 | 结果 |
|:---|:---|:---|
| 3.1 | 研究详情页自动 SSE 连接，状态指示器绿色脉冲 | ✅ |
| 3.2 | Timeline 节点按顺序激活（pending → active → done） | ✅ 状态计算引擎 |
| 3.3 | dual_search 节点显示 Web + Local 双列搜索进度 | ✅ SearchChildNode |
| 3.4 | 搜索进度显示 x/N 计数和进度条百分比 | ✅ 正则解析 message |
| 3.5 | 所有阶段完成后 Timeline 全部绿色 | ✅ COMPLETED 事件触发 |
| 3.6 | 缓存命中时显示 CacheHitBanner 而非 Timeline | ✅ 父子组件条件渲染 |
| 3.7 | SSE 断连时自动重连 | ✅ useResearchSse 管理 |
| 3.8 | 超过重连上限后显示"连接断开"状态 | ✅ SseStatusBadge |
| 3.9 | 研究失败时显示 ResearchErrorView | ✅ |
| 3.10 | 降级事件（MODEL_FALLBACK/SEARCH_FALLBACK）toast | ✅ toast id 去重 |
| 3.11 | 活跃节点耗时每秒更新 | ✅ tick useState + setInterval |
| 3.12 | lint 零错误 / typecheck 零错误 / build 6 路由成功 | ✅ |
...
| 项目 | 计划 | 实际 | 原因 |
|:---|:---|:---|:---|
| EvidenceDrawer | 右侧滑出面板 | 未实现 | 缺少后端 evidencePool API，移至 Phase 5 与历史 API 一起实现 |
| 关键发现 Tab | 从 report 解析 Finding | 占位视图 | 后端不直接返回 Findings JSON，需要 Phase 5 从历史 API 解析 |
| 引用列表 Tab | sourceIndex 表格 | 占位视图 | 依赖 evidencePool 数据，Phase 5 实现 |
| 报告数据获取 | GET /api/history/{sessionId} | GET /api/research/{sessionId} (轮询) | 复用已有轮询接口，`ResearchResponse.report` 包含完整报告 |
| rehype-highlight CSS | 计划导入 | 使用 highlight.js 默认主题（通过 rehype-highlight 内联） | rehype-highlight 自动在 `<code>` 上生成 hljs 类名和样式 |
| 右侧大纲 | 固定侧边栏 | ReportViewer 内部 xl:flex 布局 | 大纲与 ReportViewer 耦合，桌面端右侧固定，移动端隐藏 |
...
src/
├── lib/
│   └── markdown.tsx                  # ★ react-markdown 完整配置 + 10+ 自定义组件
├── hooks/
│   └── useReportData.ts              # ★ TanStack useQuery: 轮询 → 提取报告 + 标题解析
├── components/research/
│   ├── CodeBlock.tsx                 # ★ 代码块: 语言标签 + 复制按钮 + 复制动画
│   ├── CitationPopover.tsx           # ★ 引用链接: HoverCard 悬停浮窗（Phase 5 完善数据）
│   ├── ReportViewer.tsx              # ★ Tab 切换: 完整报告 / 关键发现(占位) / 引用列表(占位)
│   ├── ReportOutline.tsx             # ★ 大纲导航: 标题提取 + IntersectionObserver + 平滑滚动
│   ├── ReportSkeleton.tsx            # ★ 报告加载骨架屏（模拟 Markdown 排版）
│   └── app/(research)/research/[sessionId]/
│       └── page.tsx                  # 更新: ReportViewer + useReportData + 侧边栏大纲
```
...
| # | 标准 | 结果 |
|:---|:---|:---|
| 4.1 | Markdown 标题/段落/列表/加粗/斜体正确渲染 | ✅ react-markdown + remark-gfm |
| 4.2 | GFM 表格正确渲染 + 超宽横向滚动 | ✅ 自定义 table/thead/th/td 组件 |
| 4.3 | 代码块语法高亮 + 复制按钮 + "已复制"反馈 | ✅ rehype-highlight + CodeBlock |
| 4.4 | 引用标记 `[1]` 悬停显示 HoverCard 浮窗 | ✅ CitationPopover（数据 Phase 5 完善） |
| 4.5 | 外部链接新 Tab 打开 | ✅ `target="_blank" rel="noopener"` |
| 4.6 | 大纲自动提取 + 点击跳转 + 当前章节高亮 | ✅ IntersectionObserver + scrollIntoView |
| 4.7 | "完整报告" Tab 完整 Markdown 渲染 | ✅ |
| 4.8 | "关键发现" / "引用列表" Tab 占位提示 | ✅ Phase 5 实现 |
| 4.9 | 报告加载中 Skeleton 占位 | ✅ ReportSkeleton（模拟排版） |
| 4.10 | 报告加载失败错误提示 | ✅ |
| 4.11 | Sidebar 完成后切换为报告大纲 | ✅ 条件渲染 |
| 4.12 | lint 零错误 / typecheck 零错误 / build 成功 | ✅ |

#### Size 变化

| 路由 | Phase 3 | Phase 4 | 变化 |
|:---|:---|:---|:---|
| `/research/[sessionId]` | 11.8 kB (136 kB) | 166 kB (313 kB) | +react-markdown +highlight.js |
| 首页 `/` | 7.25 kB (135 kB) | 8.32 kB (135 kB) | 微增 |

> Phase 7 将通过 `next/dynamic` 将 ReportViewer 拆分为独立 chunk，减少首屏 JS 体积。
...
| 项目 | 计划 | 实际 | 原因 |
|:---|:---|:---|:---|
| 历史数据源 | 后端 API | 空状态优雅降级 | GET /api/history 需要后端新增 Controller，前端调 API 静默失败不报错 |
| 评估数据源 | GET /api/history/{sessionId} | 同上，poll 5s 间隔 | EvalAgent 异步执行，前端轮询等待 evalScores 写入 |
| 历史列表分页 | 后端分页（page/size） | TanStack useInfiniteQuery 已就绪 | 后端 API 实现后自动支持无限滚动 |
| EvalScoreCard 星级 | 计划用 lucide Star 图标 | 改用 ★☆ Unicode 字符 | 更轻量，视觉效果一致 |
| HistoryCard 操作按钮 | 始终可见 | hover 时显示（group-hover:opacity-100） | 减少视觉噪音，hover 交互发现 |
| `@radix-ui/react-select` | 未在 Phase 0 安装 | 新增安装 | HistoryFilters 需要 Select 组件 |
...
src/
├── hooks/
│   ├── useHistoryList.ts              # ★ TanStack useInfiniteQuery + parseEvalScores
│   └── useEvalData.ts                 # ★ 轮询 evalScores (5s interval，API 未就绪静默)
├── components/
│   ├── research/
│   │   ├── EvalScoreCard.tsx          # ★ 综合评分卡片（大数字 + 星级 + 五维进度条）
│   │   ├── EvalRadarChart.tsx         # ★ Recharts 雷达图（五维，Dark/Light 自适应）
│   │   ├── MiniRadarChart.tsx         # ★ 迷你雷达图（历史列表行内，64px/36px）
│   │   └── [sessionId]/page.tsx       # 更新: 侧边栏评估展示 + 轮询
│   ├── history/
│   │   ├── HistoryList.tsx            # ★ 完整列表（infinite scroll + 桌面表格/移动卡片 + 空状态）
│   │   ├── HistoryCard.tsx            # ★ 移动端卡片（hover 操作按钮 + 迷你雷达图）
│   │   ├── HistoryRow.tsx             # ★ 桌面端表格行（7 列 + 迷你雷达图）
│   │   ├── HistorySearch.tsx          # 300ms debounce 搜索栏
│   │   └── HistoryFilters.tsx         # 状态筛选 + 排序下拉
│   └── ui/
│       └── select.tsx                 # ★ 新增 shadcn/ui Select 组件
└── app/(research)/history/
    └── page.tsx                       # 更新: HistoryList 完整集成
```
...
| # | 标准 | 结果 |
|:---|:---|:---|
| 5.1 | 历史列表分页加载（useInfiniteQuery 已就绪） | ✅ |
| 5.2 | 关键词搜索（300ms debounce） | ✅ |
| 5.3 | 状态筛选（全部/已完成/失败） | ✅ |
| 5.4 | 排序切换（时间/字数/评分） | ✅ |
| 5.5 | 点击查看 → 跳转详情页 | ✅ |
| 5.6 | 空列表空状态（图标 + 文字 + CTA） | ✅ |
| 5.7 | 加载骨架屏（5 行 placeholder） | ✅ |
| 5.8 | 错误状态 + 重试按钮 | ✅ |
| 5.9 | 评估雷达图五维正确渲染（Recharts） | ✅ Dark/Light 自适应 |
| 5.10 | 评估分数卡片（大数字 + 星级 + 进度条） | ✅ |
| 5.11 | evalScores 为空 → "评估中..." + Skeleton | ✅ 5s 轮询等待 |
| 5.12 | 迷你雷达图历史列表行内渲染（36px） | ✅ |
| 5.13 | 删除按钮（后端未就绪静默失败） | ✅ |
| 5.14 | 移动端表格 → 卡片切换（md:block/hidden） | ✅ |
| 5.15 | lint 零错误 / typecheck 零错误 / build 成功 | ✅ |

#### 后端 API 待办

| # | 端点 | 优先级 | 前端对应 Hook |
|:---|:---|:---|:---|
| B5.1 | `GET /api/history` | P0 | `useHistoryList` |
| B5.2 | `GET /api/history/{sessionId}` | P0 | `useEvalData` |
| B5.3 | `GET /api/user/profile` | P1 | 预留 |
...
> **完成日期**: 2026-07-14 | **实际耗时**: ~2h（含后端 API 10 个端点）

#### 实施内容

前端管理后台 + **全部后端待办 API**（Phase 4-6 共 10 个端点）。
...
ui/src/
├── hooks/
│   └── usePromptManagement.ts            # ★ useQuery + useMutation (list/update/reset)
├── components/
│   ├── admin/
│   │   ├── PromptTable.tsx               # ★ 模板表格（加载/错误/空状态全覆盖）
│   │   ├── PromptEditor.tsx              # ★ Dialog 编辑器（content/status/abGroup）
│   │   └── PromptStatusBadge.tsx         # ★ 状态徽章（active/inactive/deprecated）
│   └── ui/
│       ├── dialog.tsx                    # ★ 新增 shadcn/ui Dialog 组件
│       └── label.tsx                     # ★ 新增 shadcn/ui Label 组件
└── app/(admin)/admin/prompts/
    └── page.tsx                          # 更新: PromptTable 完整集成
```
...
├── api/
│   ├── dto/
│   │   └── UpdatePromptRequest.java      # ★ DTO: content/status/abGroup (可选，含校验)
│   └── controller/
│       ├── ResearchHistoryController.java # ★ 新增: GET list + GET detail + DELETE
│       ├── UserProfileController.java    # ★ 新增: GET profile
│       └── PromptAdminController.java    # ★ 新增: GET/PUT/POST reset/POST invalidate
├── memory/
│   ├── MemoryManager.java                # 新增: getResearchHistory/getResearchBySessionId/deleteResearchHistory/getUserProfile
│   └── LongTermMemoryService.java        # 新增: getAllHistory/deleteResearchHistory/getUserProfile
└── service/
    └── DynamicPromptService.java         # 修改: loadFromClasspath private → public
```
...
| # | 端点 | 方法 | 说明 | 返回 |
|:---|:---|:---|:---|:---|
| B4.1 | `/api/history/{sessionId}` | GET | 历史详情（含完整报告+评估） | `ResearchHistory` |
| B5.1 | `/api/history` | GET | 分页查询（搜索/筛选/排序） | `Page<ResearchHistory>` |
| B5.3 | `/api/user/profile` | GET | 用户画像（统计/偏好） | `UserProfile` |
| B6.1 | `/api/admin/prompts` | GET | 全部模板列表 | `List<PromptTemplateEntity>` |
| B6.2 | `/api/admin/prompts/{id}` | GET | 单个模板详情 | `PromptTemplateEntity` |
| B6.3 | `/api/admin/prompts/{id}` | PUT | 更新模板（版本号+1） | `PromptTemplateEntity` |
| B6.4 | `/api/admin/prompts/{id}/reset` | POST | 重置为 classpath 默认值 | `PromptTemplateEntity` |
| B6.5 | `/api/admin/prompts/{id}/cache/invalidate` | POST | 强制刷新缓存 | `204` |
| B5.2 | `/api/history/{sessionId}` | DELETE | 删除历史记录 | `204` |

#### 验收结果

| # | 标准 | 结果 |
|:---|:---|:---|
| 6.1 | Prompt 列表显示全部 8 个模板 | ✅ PromptTable 完整表格 |
| 6.2 | 编辑 Dialog — content/status/abGroup 可修改 | ✅ PromptEditor |
| 6.3 | 保存后 version 自动 +1（后端 @Version） | ✅ 后端实现 |
| 6.4 | 保存 toast "已更新，1分钟内生效" | ✅ |
| 6.5 | 重置按钮 + 二次确认 + classpath 恢复 | ✅ |
| 6.6 | Status 切换 active/inactive/deprecated | ✅ |
| 6.7 | AB 分组切换 null/A/B | ✅ |
| 6.8 | 非 admin 保护（DEV_MODE 自动 admin） | ✅ |
| 6.9 | 加载/错误/空状态全覆盖 | ✅ |
| 6.10 | `DynamicPromptService.invalidateCache` 被调用 | ✅ 日志记录 |
| 6.11 | 前端 lint/typecheck/build 全部通过 | ✅ |
| 6.12 | 后端 mvn compile 零错误 | ✅ |
...
| 类别 | 内容 | 结果 |
|:---|:---|:---|
| **代码分割** | `next/dynamic` 延迟加载 ReportViewer/ReportOutline/EvalScoreCard/EvalRadarChart | `/research/[sessionId]`: 162 kB → **16.4 kB** (-90%) |
| **网络检测** | `useNetworkStatus` Hook — online/offline 事件 + Network Information API | 离线时顶部显示红色 Badge |
| **CSS 动画** | WorkflowNode 活跃节点 `scale-110` + `transition-all` + 脉冲 `boxShadow` | 替代 framer-motion（避免 v11 API 兼容问题） |
| **Suspense 包裹** | ReportViewer/EvalScoreCard/EvalRadarChart/ReportOutline 均用 `<Suspense>` | 每段独立 loading 态 |
| **前端文档** | `ui/README.md` — 完整的前端开发指南 | 包含技术栈/路由/API映射/开发规范 |
| **错误处理** | 离线 Badge + SSE 断开重连 + 报告加载失败提示 | 多层级容错覆盖 |

#### 性能对比

| 路由 | Phase 5/6 | Phase 7 | 变化 |
|:---|:---|:---|:---|
| `/` | 135 kB | 137 kB | +2 kB (NetworkStatus) |
| `/research/[sessionId]` | **430 kB** | **151 kB** | **-65% 首屏 JS** |
| `/research/[sessionId]` (page) | 162 kB | **16.4 kB** | **-90%** |
| `/admin/prompts` | 172 kB | 172 kB | 持平 |
| `/history` | 276 kB | 276 kB | 持平 |

> 核心优化：react-markdown (~150 kB) + recharts (~100 kB) 改为按需加载。用户打开研究详情页时，首屏只需加载 151 kB（原 430 kB），报告渲染组件在完成后才加载。

#### 验收结果

| # | 标准 | 结果 |
|:---|:---|:---|
| 7.1 | 响应式布局基础适配（Navbar 双行/侧边栏折叠/表格→卡片） | ✅ |
| 7.3 | `pnpm build` 零错误 + `pnpm lint` 零警告 | ✅ |
| 7.7 | `ui/README.md` 包含完整本地开发/构建/部署指南 | ✅ |
| 7.8 | CSS bundle 合理（Tailwind v4 原子化） | ✅ |
